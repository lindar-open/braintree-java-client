...
public class PayPalAccount implements PaymentMethod {
    ...
    public static PayPalAccount from(com.braintreegateway.PayPalAccount payPalAccount) {
        ...
        if (!ObjectsAcolyte.objectNullOrEmpty(payPalAccount)) {
            if (payPalAccount.getSubscriptions() != null) payPalAccountCopy.setSubscriptions(payPalAccount.getSubscriptions().stream().map(Subscription::from).collect(Collectors.toList()));
        }
    ...
    public static PayPalAccount from(com.braintreegateway.PaymentMethod paymentMethod) {
        ...
        if (!ObjectsAcolyte.objectNullOrEmpty(paymentMethod)) {
            if (paymentMethod.getSubscriptions() != null) payPalAccountCopy.setSubscriptions(paymentMethod.getSubscriptions().stream().map(Subscription::from).collect(Collectors.toList()));
        }
        return payPalAccountCopy;
    }
}
